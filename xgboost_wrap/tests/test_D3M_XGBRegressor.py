...
from xgboost_wrap import D3M_XGBRegressor
...
class TestD3M_XGBRegressor(unittest.TestCase):
    def create_learner(self, hyperparams):
        clf = D3M_XGBRegressor.D3M_XGBRegressor(hyperparams=hyperparams)
        return clf
    ...
    def test_with_semantic_types(self):
        hyperparams = D3M_XGBRegressor.Hyperparams.defaults().replace(
            {"use_semantic_types": True}
        )
        self.pickle(hyperparams)

    def test_without_semantic_types(self):
        hyperparams = D3M_XGBRegressor.Hyperparams.defaults()
        self.pickle(hyperparams)

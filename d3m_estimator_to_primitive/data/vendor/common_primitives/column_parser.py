...
class ColumnParserPrimitive(
    ...
    def _can_use_column(
        ...
        for semantic_type in self.hyperparams["parse_semantic_types"]:
            ...
            if (
                ...
                if (
                    not self.hyperparams["parse_categorical_target_columns"]
                    and "https://metadata.datadrivendiscovery.org/types/Target"
                    in semantic_types
                ):
                    continue
    ...
    def _produce_columns(
        ...
        for column_index in columns_to_use:
            ...
            if column_metadata["structural_type"] == str:
                if (
                    "http://schema.org/Boolean" in parse_semantic_types
                    and "http://schema.org/Boolean" in semantic_types
                ):
                    output_columns.append(
                        self._parse_boolean_data(inputs, column_index)
                    )

                elif (
                    "https://metadata.datadrivendiscovery.org/types/CategoricalData"
                    in parse_semantic_types
                    and "https://metadata.datadrivendiscovery.org/types/CategoricalData"
                    in semantic_types
                    and (
                        self.hyperparams["parse_categorical_target_columns"]
                        or "https://metadata.datadrivendiscovery.org/types/Target"
                        not in semantic_types
                    )
                ):
                    output_columns.append(
                        self._parse_categorical_data(inputs, column_index)
                    )

                elif (
                    "http://schema.org/Integer" in parse_semantic_types
                    and "http://schema.org/Integer" in semantic_types
                ):
                    ...
                    output_columns.append(
                        self._parse_integer(inputs, column_index, integer_required)
                    )

                elif (
                    "http://schema.org/Float" in parse_semantic_types
                    and "http://schema.org/Float" in semantic_types
                ):
                    ...
                    "https://metadata.datadrivendiscovery.org/types/FloatVector"
                    in parse_semantic_types
                    and "https://metadata.datadrivendiscovery.org/types/FloatVector"
                    in semantic_types
                ):
                    output_columns.append(
                        self._parse_float_vector_data(inputs, column_index)
                    )

                elif (
                    "http://schema.org/DateTime" in parse_semantic_types
                    and "http://schema.org/DateTime" in semantic_types
                ):
                    output_columns.append(
                        self._parse_time_data(
                            inputs, column_index, self.hyperparams["fuzzy_time_parsing"]
                        )
                    )
    ...
    def _produce_columns_metadata(
        self, inputs_metadata: metadata_base.DataMetadata
    ) -> typing.Tuple[typing.List[int], typing.List[metadata_base.DataMetadata]]:
        ...
        for column_index in columns_to_use:
            ...
            if column_metadata["structural_type"] == str:
                if (
                    ...
                    output_columns.append(
                        self._parse_boolean_metadata(inputs_metadata, column_index)
                    )
                    ...
                    output_columns.append(
                        self._parse_categorical_metadata(inputs_metadata, column_index)
                    )
                    ...
                    output_columns.append(
                        self._parse_integer_metadata(inputs_metadata, column_index)
                    )
                    ...
                    output_columns.append(
                        self._parse_float_metadata(inputs_metadata, column_index)
                    )
                    ...
                    output_columns.append(
                        self._parse_float_vector_metadata(inputs_metadata, column_index)
                    )
                    ...
                    output_columns.append(
                        self._parse_time_metadata(inputs_metadata, column_index)
                    )
    ...
    @classmethod
    def _parse_boolean_data(
        cls, inputs: Inputs, column_index: metadata_base.SimpleSelectorSegment
    ) -> Outputs:
        return cls._parse_categorical_data(inputs, column_index)

    @classmethod
    def _parse_boolean_metadata(
        cls,
        inputs_metadata: metadata_base.DataMetadata,
        column_index: metadata_base.SimpleSelectorSegment,
    ) -> metadata_base.DataMetadata:
        return cls._parse_categorical_metadata(inputs_metadata, column_index)

    @classmethod
    def _parse_categorical_data(
        cls, inputs: Inputs, column_index: metadata_base.SimpleSelectorSegment
    ) -> Outputs:
        ...
        for value in inputs.iloc[:, column_index]:
            ...
            if value not in values_map:
                ...
                values_map[value] = (
                    int.from_bytes(value_hash.digest()[0:8], byteorder="little")
                    ^ int.from_bytes(value_hash.digest()[8:16], byteorder="little")
                    ^ int.from_bytes(value_hash.digest()[16:24], byteorder="little")
                    ^ int.from_bytes(value_hash.digest()[24:32], byteorder="little")
                )

        outputs = container.DataFrame(
            {
                inputs.columns[column_index]: [
                    values_map[value.strip()] for value in inputs.iloc[:, column_index]
                ]
            },
            generate_metadata=False,
        )
        outputs.metadata = cls._parse_categorical_metadata(
    ...
    @classmethod
    def _parse_categorical_metadata(
        cls,
        inputs_metadata: metadata_base.DataMetadata,
        column_index: metadata_base.SimpleSelectorSegment,
    ) -> metadata_base.DataMetadata:
    ...
    def _parse_integer(
        ...
        outputs.metadata = outputs.metadata.update_column(
            0, {"structural_type": structural_type}
        )

        return outputs

    @classmethod
    def _parse_integer_metadata(
        cls,
        inputs_metadata: metadata_base.DataMetadata,
        column_index: metadata_base.SimpleSelectorSegment,
    ) -> metadata_base.DataMetadata:
        ...
        outputs = container.DataFrame(
            {
                inputs.columns[column_index]: [
                    container.ndarray(
                        [cls._str_to_float(value) for value in values.split(",")]
                    )
    ...
    @classmethod
    def _parse_float_vector_metadata(
        cls,
        inputs_metadata: metadata_base.DataMetadata,
        column_index: metadata_base.SimpleSelectorSegment,
    ) -> metadata_base.DataMetadata:
        ...
        outputs_metadata = outputs_metadata.update_column(
            0, {"structural_type": container.ndarray}
        )
        outputs_metadata = outputs_metadata.update(
            (metadata_base.ALL_ELEMENTS, 0, metadata_base.ALL_ELEMENTS),
